...
var conn =new tgs("ant-dashboard.i.tgcloud.io","MyGraph","tigergraph","tigergraph","bbmc2s01a1o5okcdgv32qekb6nksln84","9ujumjdq9nqooaf2d991h0shfkhuhnd4",true); 
let res=  conn.echo();
const salesData2: { x: any; y: any; }[] = [];
res.then(function(result: any) {
    let err = result["error"];
    if (err)
    {
        // problem : not connected
        console.log("Error connecting to TG")
    }
    else{
        // proceed 
        let res = conn.runQuery("ageDistribution");
        res.then(function(result: any) {
          data = result[0]['@@ageMap'];
          for (let i in data) {
              salesData2.push({
                x: i,
                y: data[i],
